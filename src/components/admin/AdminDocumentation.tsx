  ...
  },
  {
    ...
    subsections: [
      ...
      },
      {
        ...
        contentEn: '`roster_wish_seasons` is the canonical season sheet for one roster. It keeps `guild_id`, `roster_id`, lifecycle state, and dates. `class_wishes`, `external_member_wishes`, `roster_member_selection`, and `guild_season_member_intents` now reference roster-scoped season ids, so draft, active, and archived wish history is isolated per roster. `roster_season_members` stores the per-roster season snapshot, `roster_member_assignments` stores effective class/spec assignments with dated validity and can optionally reference the member character selected for that assignment, and `roster_season_events` records lightweight audit history. Managers create, activate, archive, materialize, sync deltas, and read season sheets through `prepare_roster_wish_season()`, `activate_roster_wish_season()`, `archive_roster_wish_season()`, `materialize_roster_season_members()`, `seed_roster_assignments_from_first_approved_wish()`, `apply_roster_season_sync_delta()`, `get_roster_season_table()`, `get_roster_season_history()`, and `get_roster_season_outcomes()`. Member history is returned in chronological event order by `get_roster_season_history()`. Wish activity logs keep `user_id` as the authenticated actor and `target_user_id` as the member whose wishes changed, so manager edits are auditable. Materialization seeds the current assignment from the first approved wish when the member has no active assignment yet, without changing the original wish. Managers can correct effective assignments on draft, active, or archived seasons; member wish editing stays read-only outside active seasons. Roster departures use `left_roster_at`, while true guild departures use `left_guild_at`.',
        contentFr: '`roster_wish_seasons` est la feuille de saison canonique d’un roster. Elle conserve `guild_id`, `roster_id`, l’état de cycle de vie et les dates. `class_wishes`, `external_member_wishes`, `roster_member_selection` et `guild_season_member_intents` référencent désormais des ids de saison propres au roster, afin que l’historique brouillon, actif et archivé soit isolé par roster. `roster_season_members` stocke la snapshot de saison par roster, `roster_member_assignments` stocke les affectations effectives classe/spé avec validité datée et peut référencer optionnellement le personnage du membre choisi pour cette affectation, et `roster_season_events` conserve un audit léger. Les gérants créent, activent, archivent, matérialisent, synchronisent les deltas et lisent les feuilles via `prepare_roster_wish_season()`, `activate_roster_wish_season()`, `archive_roster_wish_season()`, `materialize_roster_season_members()`, `seed_roster_assignments_from_first_approved_wish()`, `apply_roster_season_sync_delta()`, `get_roster_season_table()`, `get_roster_season_history()` et `get_roster_season_outcomes()`. L’historique membre est renvoyé dans l’ordre chronologique des événements par `get_roster_season_history()`. Les logs d’activité des vœux conservent `user_id` comme acteur authentifié et `target_user_id` comme membre dont les vœux changent, afin d’auditer les éditions gérant. La matérialisation préremplit l’affectation actuelle depuis le premier vœu approuvé quand le membre n’a pas encore d’affectation active, sans modifier le vœu d’origine. Les gérants peuvent corriger les affectations effectives sur les saisons brouillon, actives ou archivées ; l’édition des vœux membre reste en lecture seule hors saison active. Les sorties roster utilisent `left_roster_at`, tandis que les vraies sorties de guilde utilisent `left_guild_at`.',
