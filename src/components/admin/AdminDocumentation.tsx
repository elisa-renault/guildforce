  ...
  },
  {
    ...
    subsections: [
      ...
      },
      {
        ...
        contentEn: 'PostHog Cloud EU is used only for consented product analytics. Autocapture and session replay stay disabled; pageview and pageleave events are limited to PostHog web analytics. The V1 event taxonomy is `app_session_started`, `first_login`, `activated_first_action`, `wish_created`, `poll_voted`, and `guild_member_invited`; allowed properties are limited to source, feature area, guild/roster/poll identifiers, plus `$groups.guild` for guild-scoped events. Identified users receive only a deterministic pseudonymous label (`name` and `guildforce_user_label`) derived from the Supabase user id; admins can recalculate it in-app for support/debugging, and no mapping table is stored. Do not send email, BattleTag, usernames, character names, page text, poll content, comments, or secret/audit payloads. During the comparison period, Supabase `product_events` remains active for double measurement and admin operational metrics remain in Supabase.',
        contentFr: 'PostHog Cloud EU sert uniquement aux analytics produit avec consentement. L’autocapture et le session replay restent désactivés ; les événements pageview et pageleave sont limités aux web analytics PostHog. La taxonomie V1 est `app_session_started`, `first_login`, `activated_first_action`, `wish_created`, `poll_voted` et `guild_member_invited`; les propriétés autorisées sont limitées à la source, la zone fonctionnelle, les identifiants guilde/roster/sondage, plus `$groups.guild` pour les événements de guilde. Les utilisateurs identifiés reçoivent seulement un label pseudonyme déterministe (`name` et `guildforce_user_label`) dérivé de l’id utilisateur Supabase ; les admins peuvent le recalculer dans l’app pour le support/debug, sans table de correspondance stockée. Ne jamais envoyer email, BattleTag, pseudos, noms de personnages, textes de page, contenus de sondage, commentaires ou payloads de secrets/audit. Pendant la période de comparaison, Supabase `product_events` reste actif pour la double mesure et les métriques opérationnelles admin restent dans Supabase.',
