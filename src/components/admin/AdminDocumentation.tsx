...
const DOCUMENTATION: DocSection[] = [
    ...
    subsections: [
      {
        ...
        contentEn: 'Character sync writes to `wow_characters` and `wow_guild_memberships`. Profile-level main character selection is supported, and `guild_members` can store a guild-scoped main override by stable character name + realm slug for roster and member views. Sync logic includes region fallback and updates membership/rank metadata used by permissions and roster analytics.',
        contentFr: 'La synchronisation des personnages écrit dans `wow_characters` et `wow_guild_memberships`. Le choix du personnage principal au niveau profil est pris en charge, et `guild_members` peut stocker un override de main propre à la guilde via le nom de personnage + slug de royaume pour les vues roster et membres. La logique de sync inclut un fallback de région et met à jour les métadonnées de rang utilisées par les permissions et les analyses de roster.',
      ...
      },
      {
        ...
        contentEn: 'Roster composition metadata is cached in `raid_effects` for V1 major buff/debuff coverage and in `composition_abilities` + `composition_ability_mappings` for the broader utility/defensive catalog. Canonical spell text stays in `wow_spells`, refreshed from Battle.net by the `sync-wow-spells` edge function for every supported app locale. Scheduled calls must provide `x-cron-secret` from `CRON_SECRET` or use service-role authorization.',
        contentFr: 'Les métadonnées de composition de roster sont mises en cache dans `raid_effects` pour la couverture V1 des buffs/debuffs majeurs, et dans `composition_abilities` + `composition_ability_mappings` pour le catalogue plus large des utilitaires/défensifs. Le texte canonique des sorts reste dans `wow_spells`, rafraîchi depuis Battle.net par l\'edge function `sync-wow-spells` pour chaque locale supportée par l’app. Les appels planifiés doivent fournir `x-cron-secret` depuis `CRON_SECRET` ou utiliser une autorisation service-role.',
  ...
  },
  {
    ...
    subsections: [
      ...
      },
      {
        ...
        contentEn: '`guild_permissions` delegates key actions (`manage_wishes`, `manage_polls`, `manage_rosters`, `manage_members`, `view_activity_log`, `manage_vault`, `view_vault_audit`, `manage_atlas`) with `rank` and `user` rules. `manage_members` includes changing guild-scoped main characters for other members. Rank rules match Battle.net memberships using normalized realm display names and realm slugs, including accents and apostrophes. GM users always keep implicit full control. The Battle.net-tracked guild owner (`guilds.owner_id`) is also treated as an effective GM by permission helpers so a freshly claimed GM does not lose settings access if the mirrored `guild_members` role is stale.',
        contentFr: '`guild_permissions` délègue les actions clés (`manage_wishes`, `manage_polls`, `manage_rosters`, `manage_members`, `view_activity_log`, `manage_vault`, `view_vault_audit`, `manage_atlas`) avec des règles `rank` et `user`. `manage_members` inclut la modification du main de guilde des autres membres. Les règles de rang correspondent aux appartenances Battle.net via les noms de royaumes et slugs normalisés, accents et apostrophes inclus. Les GM gardent toujours un contrôle implicite complet. Le propriétaire de guilde suivi depuis Battle.net (`guilds.owner_id`) est aussi traité comme un GM effectif par les helpers de permission afin qu’un GM venant de claim sa guilde ne perde pas l’accès aux réglages si le rôle miroir dans `guild_members` est en retard.',
  ...
  },
  {
    ...
    subsections: [
      ...
      },
      {
        ...
        contentEn: '`roster_access_rules` supports `rank` ranges and explicit `user` grants. `has_roster_access()` is the canonical check used by RLS and client queries; rank access can resolve from synced `wow_guild_memberships` or linked `guild_roster_cache` rows so roster access remains stable while Battle.net membership sync catches up. `get_roster_access_debug()` exposes the resolved source and best rank for admin diagnostics.',
        contentFr: '`roster_access_rules` supporte les plages de `rank` et les accès `user` explicites. `has_roster_access()` est le contrôle canonique utilisé par les politiques RLS et les requêtes côté client ; l’accès par rang peut être résolu depuis `wow_guild_memberships` synchronisé ou les lignes liées de `guild_roster_cache` afin que l’accès roster reste stable pendant le rattrapage de la synchronisation Battle.net. `get_roster_access_debug()` expose la source résolue et le meilleur rang pour le diagnostic admin.',
  ...
  },
  {
    ...
    subsections: [
      ...
      },
      {
        ...
        contentEn: '`roster_wish_seasons` is the canonical season sheet for one roster. It keeps `guild_id`, `roster_id`, lifecycle state, dates, and `hide_member_wishes` for per-season wish privacy. `class_wishes`, `external_member_wishes`, `roster_member_selection`, and `guild_season_member_intents` reference roster-scoped season ids, so draft, active, and archived wish history is isolated per roster. `roster_season_members` stores the per-roster season snapshot and `roster_season_events` records lightweight audit history; materialization upserts use roster-scoped unique targets (`roster_id`, `season_id`, `user_id`) and (`roster_id`, `season_id`, `roster_cache_id`). `roster_season_event_type` is retained as a compatibility enum for RPC casts while `roster_season_events.event_type` remains text. `upsert_member_roster_wishes()` also ensures the linked member season snapshot exists, so a member\'s first saved wish is immediately visible through `get_roster_season_table()`. A cache-claim trigger merges stale external season-member rows when a `guild_roster_cache` row becomes linked to a Guildforce user, preventing duplicate cache-key failures during materialization. The retired assignment seeding RPC remains only as a no-op compatibility shim, and `get_roster_season_table()` keeps `current_assignment` as nullable compatibility data without reading retired assignment tables. Managers create rosters with their initial draft season through `create_roster_with_wish_season()`, and existing rosters without seasons are backfilled with an initial draft and member snapshot. Managers then activate, archive, materialize, sync deltas, and read season sheets through `prepare_roster_wish_season()`, `activate_roster_wish_season()`, `archive_roster_wish_season()`, `materialize_roster_season_members()`, `apply_roster_season_sync_delta()`, `get_roster_season_table()`, `get_roster_season_history()`, and `get_roster_season_outcomes()`. Archived seasons can be reactivated with `activate_roster_wish_season()` after a mistaken archive, and GM/`manage_wishes` users can correct archived wish sheets for data recovery through the manager RPCs. Member history is returned in chronological event order by `get_roster_season_history()`, which wraps the unioned event sources before applying timestamp order. Wish activity logs keep `user_id` as the authenticated actor and `target_user_id` as the member whose wishes changed, so manager edits are auditable. Member self-service editing stays read-only outside active seasons. Roster departures use `left_roster_at`, while true guild departures use `left_guild_at`.',
        contentFr: '`roster_wish_seasons` est la feuille de saison canonique d’un roster. Elle conserve `guild_id`, `roster_id`, l’état de cycle de vie, les dates et `hide_member_wishes` pour la confidentialité des vœux par saison. `class_wishes`, `external_member_wishes`, `roster_member_selection` et `guild_season_member_intents` référencent des ids de saison propres au roster, afin que l’historique brouillon, actif et archivé soit isolé par roster. `roster_season_members` stocke la snapshot de saison par roster et `roster_season_events` conserve un audit léger ; la matérialisation utilise des cibles uniques propres au roster (`roster_id`, `season_id`, `user_id`) et (`roster_id`, `season_id`, `roster_cache_id`). `roster_season_event_type` reste un enum de compatibilité pour les casts RPC, tandis que `roster_season_events.event_type` demeure en texte. `upsert_member_roster_wishes()` garantit aussi la présence de la snapshot de saison du membre lié, afin que son premier vœu sauvegardé soit immédiatement visible via `get_roster_season_table()`. Un trigger de claim du cache fusionne les anciennes lignes externes de saison lorsqu’une ligne `guild_roster_cache` est rattachée à un utilisateur Guildforce, afin d’éviter les échecs de clé cache dupliquée pendant la matérialisation. L’ancien RPC de seeding des assignments reste seulement comme shim de compatibilité no-op, et `get_roster_season_table()` conserve `current_assignment` comme donnée nullable de compatibilité sans lire les anciennes tables d’assignments. Les gérants créent les rosters avec leur saison brouillon initiale via `create_roster_with_wish_season()`, et les rosters existants sans saison sont rattrapés avec un brouillon initial et une snapshot membre. Les gérants activent, archivent, matérialisent, synchronisent les deltas et lisent ensuite les feuilles via `prepare_roster_wish_season()`, `activate_roster_wish_season()`, `archive_roster_wish_season()`, `materialize_roster_season_members()`, `apply_roster_season_sync_delta()`, `get_roster_season_table()`, `get_roster_season_history()` et `get_roster_season_outcomes()`. Une saison archivée peut être réactivée avec `activate_roster_wish_season()` après une archive accidentelle, et les GM/`manage_wishes` peuvent corriger les feuilles de vœux archivées pour reprise de données via les RPCs gérant. L’historique membre est renvoyé dans l’ordre chronologique des événements par `get_roster_season_history()`, qui enveloppe les sources unionées avant d’appliquer l’ordre par date. Les logs d’activité des vœux conservent `user_id` comme acteur authentifié et `target_user_id` comme membre dont les vœux changent, afin d’auditer les éditions gérant. L’édition libre par les membres reste en lecture seule hors saison active. Les sorties roster utilisent `left_roster_at`, tandis que les vraies sorties de guilde utilisent `left_guild_at`.',
  ...
  },
  {
    ...
    subsections: [
      ...
      },
      {
        ...
        contentEn: 'Admin UI covers dashboard, users, permissions, guilds, legal pages, patch notes, bug reports, automatic client error reports, deletion requests, documentation, and backup/export tools.',
        contentFr: 'L\'interface admin couvre tableau de bord, utilisateurs, permissions, guildes, pages légales, patch notes, rapports de bugs, rapports automatiques d\'erreurs client, demandes de suppression, documentation et outils de sauvegarde/export.',
        tags: ['admin', 'sections'],
      },
      {
  ...
  },
  {
    ...
    subsections: [
      ...
      {
        titleEn: 'Wish activity log visibility',
        titleFr: 'Visibilité des logs de vœux',
        contentEn: '`guild_activity_logs` is not broadly readable by guild members. Wish and roster-decision rows carry `roster_id` and `season_id` so history can be scoped exactly to one roster season. Users with `view_activity_log` keep full guild audit access, while regular members can only read their own wish activity rows (`wish_created`, `wish_updated`, `wish_deleted`, `wish_validation`, `roster_selection_changed`) through `target_user_id = auth.uid()`.',
        contentFr: '`guild_activity_logs` n’est pas lisible largement par tous les membres de guilde. Les lignes de vœux et de décision portent `roster_id` et `season_id` pour rattacher l’historique précisément à une saison de roster. Les utilisateurs avec `view_activity_log` gardent l’accès audit complet de la guilde, tandis que les membres standards peuvent seulement lire leurs propres lignes d’activité de vœux (`wish_created`, `wish_updated`, `wish_deleted`, `wish_validation`, `roster_selection_changed`) via `target_user_id = auth.uid()`.',
        tags: ['security', 'rls', 'wishes', 'activity'],
      },
      {
      ...
      },
      {
        ...
        contentEn: 'PostHog Cloud EU is used only for consented product analytics and consent-aware error visibility. Autocapture and session replay stay disabled; pageview and pageleave events are limited to PostHog web analytics, while JavaScript exception capture is routed through sanitized PostHog error helpers. The V1 event taxonomy is `app_session_started`, `first_login`, `activated_first_action`, `wish_created`, `poll_voted`, and `guild_member_invited`; allowed properties are limited to source, feature area, guild/roster/poll identifiers, plus `$groups.guild` for guild-scoped events. PostHog kill switches may disable Atlas, Vault, poll AI summaries, or roster seasons, but they must not replace Supabase authorization/RLS checks. Identified users receive only a deterministic pseudonymous label (`name` and `guildforce_user_label`) derived from the Supabase user id; admins can recalculate it in-app for support/debugging, and no mapping table is stored. Do not send email, BattleTag, usernames, character names, page text, poll content, comments, secret/audit payloads, or raw stack content. During the comparison period, Supabase `product_events` remains active for double measurement and admin operational metrics remain in Supabase.',
        contentFr: 'PostHog Cloud EU sert uniquement aux analytics produit avec consentement et à une visibilité d’erreurs conditionnée au consentement. L’autocapture et le session replay restent désactivés ; les événements pageview et pageleave sont limités aux web analytics PostHog, tandis que la capture d’exceptions JavaScript passe par des helpers PostHog filtrés. La taxonomie V1 est `app_session_started`, `first_login`, `activated_first_action`, `wish_created`, `poll_voted` et `guild_member_invited`; les propriétés autorisées sont limitées à la source, la zone fonctionnelle, les identifiants guilde/roster/sondage, plus `$groups.guild` pour les événements de guilde. Les kill switches PostHog peuvent désactiver Atlas, le coffre, les résumés IA de sondages ou les saisons de roster, mais ils ne remplacent jamais les contrôles d’autorisation/RLS Supabase. Les utilisateurs identifiés reçoivent seulement un label pseudonyme déterministe (`name` et `guildforce_user_label`) dérivé de l’id utilisateur Supabase ; les admins peuvent le recalculer dans l’app pour le support/debug, sans table de correspondance stockée. Ne jamais envoyer email, BattleTag, pseudos, noms de personnages, textes de page, contenus de sondage, commentaires, payloads de secrets/audit ou contenu brut de stack. Pendant la période de comparaison, Supabase `product_events` reste actif pour la double mesure et les métriques opérationnelles admin restent dans Supabase.',
