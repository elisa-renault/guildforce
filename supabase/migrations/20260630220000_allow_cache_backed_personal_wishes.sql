-- Allow personal wish edits while guild_members lags behind roster-cache matching.
-- Battle.net roster cache can already grant has_roster_access; this keeps the
-- save path aligned with that access model without creating guild_members rows.
...
BEGIN
  ...
  IF NOT v_member_exists
    AND NOT (
      v_actor = p_member_id
      AND public.has_roster_access(p_roster_id, p_member_id)
    ) THEN
  ...
  best_cache AS (
    ...
    ORDER BY grc.matched_user_id, grc.rank_index ASC NULLS LAST, grc.updated_at DESC
  ),
  member_source AS (
    SELECT
      p_member_id AS user_id,
      gm.wishes_locked,
      p.username,
      COALESCE(em.roster_cache_id, bc.roster_cache_id) AS roster_cache_id,
      COALESCE(em.character_name, bc.character_name) AS character_name,
      COALESCE(em.character_realm, bc.character_realm, bc.character_realm_slug) AS realm_name,
      bc.rank_index
    FROM public.profiles p
    LEFT JOIN public.guild_members gm
      ON gm.guild_id = p_guild_id
     AND gm.user_id = p.id
    LEFT JOIN effective_main em ON em.user_id = p.id
    LEFT JOIN best_cache bc ON true
  ...
  member_snapshot AS (
    SELECT
      ms.user_id,
      ms.roster_cache_id,
      COALESCE(ms.character_name, ms.username, ms.user_id::text) AS display_name,
      COALESCE(ms.character_name, ms.username) AS character_name,
      ms.realm_name,
      ms.rank_index,
      CASE
