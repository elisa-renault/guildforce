-- External members: editable commitment status and transfer to claimed account.

ALTER TABLE public.external_member_wishes
ADD COLUMN IF NOT EXISTS commitment_status TEXT NOT NULL DEFAULT 'potential'
CHECK (commitment_status IN ('confirmed', 'potential', 'withdrawn'));

DROP FUNCTION IF EXISTS public.upsert_external_member_wish(UUID, UUID, TEXT, TEXT[], TEXT);
