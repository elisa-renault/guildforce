-- Keep admin product activity metrics tied to explicit user actions.
...
COMMENT ON FUNCTION public.get_admin_dashboard_stats()
  IS 'Admin dashboard KPI snapshot. Product activity windows intentionally exclude Battle.net membership sync/backfill timestamps.';

COMMENT ON FUNCTION public.get_admin_dashboard_timeseries(integer)
  IS 'Admin dashboard daily trend points. Product activity uses explicit user actions and excludes Battle.net membership sync/backfill timestamps.';
