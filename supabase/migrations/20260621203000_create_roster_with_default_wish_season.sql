-- Create rosters and their initial draft wish season atomically.

CREATE OR REPLACE FUNCTION public.create_roster_with_wish_season(
  p_guild_id UUID,
  p_name TEXT,
  p_description TEXT DEFAULT NULL,
  p_access_rules JSONB DEFAULT '[]'::jsonb,
  p_season_name TEXT DEFAULT 'Midnight - Saison 1'
)
...
BEGIN
  ...
  IF p_guild_id IS NULL THEN
    RAISE EXCEPTION 'Guild is required' USING ERRCODE = '22023';
  END IF;
  ...
  IF jsonb_typeof(COALESCE(p_access_rules, '[]'::jsonb)) <> 'array' THEN
    RAISE EXCEPTION 'Access rules must be an array' USING ERRCODE = '22023';
  END IF;

  IF NOT (
    public.is_guild_gm(p_guild_id, v_actor)
    OR public.has_guild_permission(p_guild_id, v_actor, 'manage_rosters')
  ) THEN
    RAISE EXCEPTION 'Not authorized' USING ERRCODE = '42501';
  END IF;

  INSERT INTO public.rosters (guild_id, name, description)
  VALUES (p_guild_id, trim(p_name), NULLIF(trim(COALESCE(p_description, '')), ''))
  RETURNING * INTO v_roster;

  FOR v_rule IN SELECT value FROM jsonb_array_elements(COALESCE(p_access_rules, '[]'::jsonb))
  LOOP
    v_access_type := v_rule->>'access_type';

    IF v_access_type NOT IN ('rank', 'user') THEN
      RAISE EXCEPTION 'Invalid access rule type' USING ERRCODE = '22023';
    END IF;

    INSERT INTO public.roster_access_rules (
      roster_id,
      access_type,
      user_id,
      min_rank_index,
    ...
    VALUES (
      ...
      v_access_type,
      CASE
        WHEN v_access_type = 'user' THEN NULLIF(v_rule->>'user_id', '')::UUID
        ELSE NULL
      END,
      CASE
        WHEN v_access_type = 'rank' THEN NULLIF(v_rule->>'min_rank_index', '')::INTEGER
        ELSE NULL
      END,
      CASE
        WHEN v_access_type = 'rank' THEN NULLIF(v_rule->>'max_rank_index', '')::INTEGER
        ELSE NULL
      END
    );
  END LOOP;
  ...
  VALUES (
    p_guild_id,
    v_roster.id,
    trim(COALESCE(NULLIF(p_season_name, ''), 'Midnight - Saison 1')),
    'draft'::public.guild_season_state,
    v_actor
  )
  RETURNING id INTO v_season_id;

  PERFORM public.materialize_roster_season_members(v_roster.id, v_season_id);
...
BEGIN
  ...
  inserted_linked AS (
    ...
    ON CONFLICT (season_id, user_id) WHERE user_id IS NOT NULL
    DO UPDATE SET
  ...
  inserted_external AS (
    ...
    ON CONFLICT (season_id, roster_cache_id) WHERE roster_cache_id IS NOT NULL
    DO UPDATE SET
      display_name_snapshot = EXCLUDED.display_name_snapshot,
      character_name_snapshot = EXCLUDED.character_name_snapshot,
      realm_snapshot = EXCLUDED.realm_snapshot,
